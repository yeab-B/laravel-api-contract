<?php

declare(strict_types=1);

namespace Yab\LaravelApiContract\Tests\Feature\Console;

use Illuminate\Support\Facades\Route;
use Yab\LaravelApiContract\Tests\Support\TestControllers\UserResourceController;
use Yab\LaravelApiContract\Tests\TestCase;

class TypeScriptCommandTest extends TestCase
{
    private string $outputPath;

    protected function setUp(): void
    {
        parent::setUp();

        $this->outputPath = tempnam(sys_get_temp_dir(), 'typescript-test-');
    }

    protected function tearDown(): void
    {
        if (file_exists($this->outputPath)) {
            unlink($this->outputPath);
        }

        parent::tearDown();
    }

    protected function defineRoutes($router): void
    {
        Route::middleware('api')->prefix('api')->group(function () {
            Route::get('users', [UserResourceController::class, 'index']);
            Route::post('users', [UserResourceController::class, 'store']);
        });
    }

    public function test_command_succeeds(): void
    {
        $this->artisan('api-contract:typescript')
            ->assertSuccessful();
    }

    public function test_command_writes_to_file(): void
    {
        $this->artisan('api-contract:typescript', ['--output' => $this->outputPath])
            ->assertSuccessful();

        $this->assertFileExists($this->outputPath);

        $content = file_get_contents($this->outputPath);
        $this->assertStringContainsString('export interface', $content);
    }

    public function test_generated_typescript_has_interface(): void
    {
        $this->artisan('api-contract:typescript', ['--output' => $this->outputPath])
            ->assertSuccessful();

        $content = file_get_contents($this->outputPath);

        $this->assertStringContainsString('export interface', $content);
    }

    public function test_generated_typescript_contains_auto_generated_header(): void
    {
        $this->artisan('api-contract:typescript', ['--output' => $this->outputPath])
            ->assertSuccessful();

        $content = file_get_contents($this->outputPath);

        $this->assertStringContainsString('Auto-generated by laravel-api-contract', $content);
    }
}
